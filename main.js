...
//drawing the text and positioning the particles
ctx.fillStyle = 'rgba(255,255,255,0.8)';
ctx.font = '20px Arial';
ctx.fillText('Stimming', 0, 30);
//scans 500x500 pixels of the text to find pixels with any opacity
        ...
        for (let y = 0, y2 = textCoordinates.height; y < y2; y++) {
            for (let x = 0, x2 = textCoordinates.width; x < x2; x++) {
                if (textCoordinates.data[(y * 4 * textCoordinates.width)
                    ...
                    let positionX = x;
                    let positionY = y;
                    particleArray.push(new particle(positionX * 15,
                        positionY * 15 ));
                }
            }
        }
